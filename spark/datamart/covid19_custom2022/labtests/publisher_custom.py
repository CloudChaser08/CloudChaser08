...
class Covid19LabPublisher:
    ...
    _all_tests_table_custom = context.LAB_FACT_ALL_TESTS_CUSTOM
    _all_tests_db_table_custom = '{}.{}'.format(_dw_db, _all_tests_table_custom)

    _covid_tests_table = context.LAB_FACT_COVID_TESTS_CUSTOM
    ...
    def create_table_if_not_and_repair(self, spark, runner):
        ...
        for table in self._tables_list:
            ...
            if table in [self._all_tests_table, self._all_tests_table_custom, self._covid_tests_table]:
            ...
            if not dmutil.has_table(spark, self._dw_db, table):
                runner.run_spark_script(table_sql, [
                    ['table_location', table_location.rstrip('/') + '/' + table + '/']
