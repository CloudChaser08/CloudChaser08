SELECT
    ...
    ,test.hv_method_flag
    ,CASE
         WHEN test_ordered_std_id IN ('39504','39504X','0039504','0039728','TH99')                 THEN 'COVID Antibody'
    ...
    ,CASE
        WHEN hv_method_flag = 1 THEN
            CASE
                ...
                WHEN LOWER(result) IN ('cdcdet','detect','ldtdet','p','p=','pos') THEN '01 Positive'
                ...
                WHEN LOWER(result) LIKE 'not detcted%'                            THEN '03 Negative'
                WHEN LOWER(result) LIKE 'none detect%'                            THEN '03 Negative'
                WHEN LOWER(result) IN ('neg','nd','no','none','not','none seen')  THEN '03 Negative'
                WHEN LOWER(result) IN ('cdcnot','ldtnot','n','n=','notdet')       THEN '03 Negative'
                ...
                WHEN LOWER(result) IN ('cdcind','eqr','indete','ldtind','pcrinh') THEN '04 Inconclusive'
                ELSE NULL
            END
...
FROM
    lab_cleanse_covid_tests_all test
WHERE
    test.hv_method_flag IN (1,2)
GROUP BY
    test.part_provider
    ,test.test_ordered_name
    ,test.result_name
