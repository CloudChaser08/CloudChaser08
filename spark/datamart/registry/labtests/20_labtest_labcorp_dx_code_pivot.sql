...
--   transactions txn
--   JOIN results rslt ON txn.specimen_number = rslt.specimen_number  --  for performance tuning
  CROSS JOIN (SELECT EXPLODE(ARRAY(0, 1, 2, 3, 4, 5)) AS n)  dx_explode
WHERE
--   EXISTS(SELECT 1 FROM transactions txn WHERE txn.specimen_number = rslt.specimen_number) --for performance tuning change in DA-1881
--   AND
