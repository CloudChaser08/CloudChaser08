...
SELECT
    ...
    regexp_replace(c.transaction_date, '(?<day>[0-9]{2})/(?<month>[0-9]{2})/(?<year>[0-9]{4})', '$3-$1-$2 00:00:00') as transaction_date,
    regexp_replace(c.expiry_date, '(?<day>[0-9]{2})/(?<month>[0-9]{2})/(?<year>[0-9]{4})', '$3-$1-$2 00:00:00') as expiry_date,
    c.status as status,
