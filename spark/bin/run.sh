...
spark-submit --py-files $prefix/target/dewey.zip --jars $prefix/common/json-serde-1.3.7-jar-with-dependencies.jar,$prefix/common/HiveJDBC41.jar --conf spark.driver.memory=10G --conf spark.executor.cores=4 --spark.sql.shuffle.partitions=2001 --conf spark.file.useFetchCache=false --conf spark.hadoop.fs.s3a.connection.maximum=1000 $mem_cfg $@
