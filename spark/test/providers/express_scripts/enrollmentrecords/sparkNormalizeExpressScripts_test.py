import pytest
import datetime
import spark.providers.express_scripts.enrollment.sparkNormalizeExpressScriptsEnrollment as esi
...
def test_no_orphaned_records():
    ...
    assert len(filter(lambda r: r.start_date == datetime.date(2017, 4, 22))) == 0

def test_start_date_parsing():
    "Check that start_date is parsed and not capped even when it's absurd"
    assert len(filter(lambda r: r.start_date == datetime.date(1856, 1, 1))) != 0

def test_end_date_parsing():
    "Check that start_date is parsed and not capped even when it's absurd"
    assert len(filter(lambda r: r.start_date == datetime.date(3000, 1, 1))) != 0
