...
def test_no_orphaned_records():
    ...
    assert len(filter(lambda r: r.date_start == datetime.date(2017, 4, 22), results)) == 0

def test_date_start_parsing():
    "Check that date_state is parsed and not capped even when it's absurd"
    assert len(filter(lambda r: r.date_start == datetime.date(1901, 1, 1), results)) != 0

def test_end_date_parsing():
    "Check that date_end is parsed and not capped even when it's absurd"
    assert len(filter(lambda r: r.date_end == datetime.date(3000, 1, 1), results)) != 0
