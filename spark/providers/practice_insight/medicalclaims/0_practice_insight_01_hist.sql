SELECT 
    ...
    procedure_code_qual,
    principal_proc_ind,
    CAST(procedure_units AS FLOAT) AS procedure_units,
    ...
    CAST(line_charge AS FLOAT) AS line_charge,
    CAST(line_allowed AS FLOAT) AS line_allowed,
    CAST(total_charge AS FLOAT) AS total_charge,
    CAST(total_allowed AS FLOAT) AS total_allowed,
    prov_rendering_npi,
    prov_billing_npi,
    prov_referring_npi,
    prov_facility_npi,
    ...
    part_best_date
FROM _temp_medicalclaims_nb
WHERE
    TRUE
    AND part_best_date >= SUBSTR(ADD_MONTHS(TRUNC(CAST('{VDR_FILE_DT}' AS DATE), 'MONTH'), -2),1,7)
    AND part_best_date <= SUBSTR(ADD_MONTHS(TRUNC(CAST('{VDR_FILE_DT}' AS DATE), 'MONTH'),  0),1,7)
