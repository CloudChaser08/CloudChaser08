   ...
   (
        ...
        CASE
            WHEN EXTRACT_DATE(transactional.svc_from_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)) IS NOT NULL
                AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                THEN CAST(extract_date(transactional.svc_from_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)) AS DATE)
            WHEN EXTRACT_DATE(transactional.stmnt_from_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)) IS NOT NULL
                THEN CAST(extract_date(transactional.stmnt_from_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)) AS DATE)
            ELSE MIN(CAST(EXTRACT_DATE(transactional.svc_from_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)) AS DATE))
    ...
    EXTRACT_DATE(
        ...
        CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)
        )                                                   AS date_received,
    CASE
        WHEN EXTRACT_DATE(transactional.svc_from_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date)) IS NOT NULL
                AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
    ...
    CASE
        WHEN transactional.claim_type_cd = 'I'
            THEN extract_date(transactional.admsn_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date))
    END                                                     AS inst_date_admitted,
    CASE
        WHEN transactional.claim_type_cd = 'I'
            THEN extract_date(transactional.dischg_dt, '%Y%m%d', CAST('{EARLIEST_SERVICE_DATE}' as date), CAST('{VDR_FILE_DT}' as date))
    ...
    CASE
        WHEN transactional.claim_type_cd == 'P' THEN
            CASE
                WHEN transactional.pos_cd IS NOT NULL AND transactional.pos_cd != ''
                        AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                    THEN transactional.pos_cd
                WHEN transactional.fclty_type_pos_cd != '' AND transactional.fclty_type_pos_cd IS NOT NULL
                    THEN transactional.fclty_type_pos_cd
                ELSE NULL
            END
        ELSE NULL
    ...
    CASE
        WHEN diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
    ...
    CASE
        WHEN diags.diag_code NOT IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4) OR transactional.claim_type_cd = 'I'
            THEN NULL
    ...
    CASE
        WHEN transactional.claim_type_cd != 'I'
                AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                AND transactional.rendr_provdr_npi_svc IS NOT NULL AND TRIM(transactional.rendr_provdr_npi_svc) <> ''
    ...
    CASE
        WHEN transactional.claim_type_cd != 'I'
                AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                AND transactional.refrn_provdr_npi_svc IS NOT NULL AND TRIM(transactional.refrn_provdr_npi_svc) <> ''
    ...
    CASE
        WHEN transactional.claim_type_cd != 'I'
                AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                AND transactional.fclty_npi_svc IS NOT NULL AND TRIM(transactional.fclty_npi_svc) <> ''
    ...
    CASE
        WHEN transactional.claim_type_cd = 'I'
                OR diags.diag_code NOT IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                OR transactional.rendr_provdr_npi_svc IS NULL OR transactional.rendr_provdr_npi_svc = ''
    ...
    CASE
        WHEN transactional.claim_type_cd = 'I'
                OR diags.diag_code NOT IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                OR transactional.refrn_provdr_npi_svc IS NULL OR transactional.refrn_provdr_npi_svc = ''
    ...
    CASE
        WHEN transactional.claim_type_cd = 'I'
                OR diags.diag_code NOT IN (transactional.diag_cd_1, transactional.diag_cd_2, transactional.diag_cd_3, transactional.diag_cd_4)
                OR transactional.fclty_npi_svc IS NULL OR transactional.fclty_npi_svc = ''
    ...
    'practice_insight'                                      AS part_provider
FROM txn transactional
    LEFT OUTER JOIN matching_payload mp ON transactional.src_claim_id = mp.claimid
-- these inner joins will each perform a cartesian product on this table, exploding the table for each diag/proc
    INNER JOIN practice_insight_03_clm_diag diags ON CONCAT(COALESCE(transactional.src_claim_id, ''), '__', COALESCE(transactional.src_svc_id, '')) = diags.claim_svc_num
    INNER JOIN practice_insight_05_clm_proc procs ON CONCAT(COALESCE(transactional.src_claim_id, ''), '__', COALESCE(transactional.src_svc_id, '')) = procs.claim_svc_num
