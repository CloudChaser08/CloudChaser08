...
SELECT DISTINCT  
    ...
    CAST(transactional.edi_interchange_creation_dt as DATE),                -- date_received
    CASE 
    WHEN CAST(transactional.svc_from_dt AS DATE) IS NOT NULL 
    AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, 
        transactional.diag_cd_3, transactional.diag_cd_4)
    THEN CAST(transactional.svc_from_dt as DATE)
    WHEN transactional.stmnt_from_dt IS NOT NULL 
    THEN CAST(transactional.stmnt_from_dt as DATE)
    ELSE (
    SELECT MIN(CAST(t2.svc_from_dt as DATE))
    ...
    AND diags.diag_code IN (transactional.diag_cd_1, transactional.diag_cd_2, 
        transactional.diag_cd_3, transactional.diag_cd_4)
    THEN CAST(transcational.svc_to_dt AS DATE)
    WHEN CAST(transactional.stmnt_from_dt AS DATE) IS NOT NULL
    THEN CAST(transactional.stmnt_to_dt AS DATE)
    ELSE (
    SELECT MAX(CAST(t2.svc_to_dt AS DATE))
    FROM transactional_raw t2
    WHERE t2.src_claim_id = transactional.src_claim_id
        ) 
    ...
    THEN CAST(transactional.admsn_dt as DATE)
    END,                                                                    -- inst_date_admitted
    CASE
    WHEN transactional.claim_type_cd = 'I'
    THEN CAST(transactional.dischg_dt as DATE)
    ...
    clean_up_diagnosis_code(
        diags.diag_code, NULL, CAST(date_service as DATE)
        ),  -- diagnosis_code
...
SELECT DISTINCT
    ...
    CASE 
    WHEN CAST(transactional.svc_from_dt AS DATE) IS NOT NULL 
    THEN CAST(transactional.svc_to_dt AS DATE)
    WHEN CAST(transactional.stmnt_from_dt AS DATE) IS NOT NULL
    THEN CAST(transactional.stmnt_to_dt AS DATE)
    ELSE (
    SELECT MAX(CAST(transactional.svc_to_dt AS DATE))
    FROM transactional_raw t2
    WHERE t2.src_claim_id = transactional.src_claim_id
        ) 
    END,                                                                               -- date_service_end
    CAST(transactional.admsn_dt AS DATE),                                              -- inst_date_admitted
    CAST(transactional.dischg_dt AS DATE),                                             -- inst_date_discharged
    ...
    NULL,                                                                              -- inst_discharge_status_vendor_id
    NULL,                                                                              -- inst_discharge_status_vendor_desc
    CONCAT(transactional.fclty_type_pos_cd, transactional.claim_freq_cd),              -- inst_type_of_bill_std_id
    NULL,                                                                              -- inst_type_of_bill_vendor_id
    NULL,                                                                              -- inst_type_of_bill_vendor_desc
