SELECT
 ...
 FROM (
        SELECT
         ...
         FROM erx_04_pot_revd
        ) AS rvd
WHERE --rvd.row_num = 1 AND 
    EXISTS
    (
         SELECT 1
         FROM erx_02_norm rvs
        WHERE COALESCE(rvs.logical_delete_reason, '') = 'Reversal'
          AND COALESCE(rvd.rx_number, '') = COALESCE(rvs.rx_number, '')
          AND COALESCE(rvd.pharmacy_npi, '') = COALESCE(rvs.pharmacy_npi, '')
          AND COALESCE(rvd.ndc_code) = COALESCE(rvs.ndc_code, '')
          AND COALESCE(rvd.bin_number, '') = COALESCE(rvs.bin_number, '')
          AND COALESCE(CAST(rvd.fill_number AS INT), '') = COALESCE(CAST(rvs.fill_number AS INT), '')
    )
UNION
SELECT
     record_id,
