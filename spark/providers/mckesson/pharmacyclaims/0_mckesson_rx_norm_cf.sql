SELECT
    ...
    MASK_ZIP_CODE(mp.threeDigitZip)                                 AS patient_zip3,
    VALIDATE_STATE_CODE(TRIM(UPPER(mp.state)))                      AS patient_state,
    EXTRACT_DATE(
    ...
    CLEAN_UP_PROCEDURE_CODE(t.procedurecode)                        AS procedure_code,
    t.procedurecodequalifier                                        AS procedure_code_qual,
    CLEAN_UP_NDC_CODE(t.dispensedndcnumber)                         AS ndc_code,
    ...
    CLEAN_UP_NPI_CODE(t.primarycareproviderid)                      AS prov_primary_care_npi,
