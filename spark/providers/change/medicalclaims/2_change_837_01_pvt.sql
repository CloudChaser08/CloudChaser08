SELECT
   ...
   clm.rendering_attending_prov_taxonomy			                           AS prov_rendering_std_taxonomy, 
    ...
    ---------------------------------------------------------------------------------------------------------------------------    
    'end'
FROM change_837_00_srv  sln
LEFT OUTER JOIN change_837_00_clm       clm ON  UPPER(sln.claim_number) = UPPER(clm.claim_tcn_id)
-- LEFT OUTER JOIN change_837_payload     pay ON  UPPER(sln.claim_number) = UPPER(pay.claimid)
-- LEFT OUTER JOIN change_837_passthrough pas ON  UPPER(sln.claim_number) = UPPER(pas.claimid)
-- LEFT OUTER JOIN change_837_plainout    pln ON  UPPER(sln.claim_number) = UPPER(pln.claim_number)
