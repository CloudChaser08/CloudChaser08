SELECT
    ...
    CASE 
        WHEN TO_DATE(COALESCE(clm.statement_from_date, min_max_dt.min_service_from_date), 'yyyyMMdd')  < '{EARLIEST_SERVICE_DATE}' 
    ...
    END                                                                                      AS date_service,
    CASE 
        WHEN TO_DATE(COALESCE(clm.statement_to_date,min_max_dt.max_service_to_date), 'yyyyMMdd')  < '{EARLIEST_SERVICE_DATE}' 
          -- OR TO_DATE(COALESCE(clm.statement_to_date,min_max_dt.max_service_to_date), 'yyyyMMdd') > '{VDR_FILE_DT}'
          THEN NULL
    ...
    ---------------------------------------------------------------------------------------------------------------------------
    -- Diag There are instances where all diag codes are NULL except admitting_diagnosis_code, so a new CASE is added
    ---------------------------------------------------------------------------------------------------------------------------
      CASE
        WHEN ARRAY
                (
                clm.principal_diagnosis_code, clm.other_diagnosis_code_1, clm.other_diagnosis_code_2, clm.other_diagnosis_code_3,
                clm.other_diagnosis_code_4  , clm.other_diagnosis_code_5, clm.other_diagnosis_code_6, clm.other_diagnosis_code_7
                )[diag_explode.n] IS NULL 
             AND clm.admitting_diagnosis_code IS NULL THEN NULL
        ...
        WHEN ARRAY
             ...
             AND clm.admitting_diagnosis_code IS NOT NULL THEN clm.admitting_diagnosis_code
    ...
    CASE 
        ...
        WHEN COALESCE(claim_type, 'X') = 'I' AND clm.admitting_diagnosis_code = ARRAY
                (
                clm.principal_diagnosis_code, clm.other_diagnosis_code_1, clm.other_diagnosis_code_2, clm.other_diagnosis_code_3,
                clm.other_diagnosis_code_4  , clm.other_diagnosis_code_5, clm.other_diagnosis_code_6, clm.other_diagnosis_code_7
                )[diag_explode.n] THEN  'Y' 
    ELSE NULL 
...
(
    ...
    FROM change_837_00_srv line_1
    GROUP BY 1
) min_max_dt    ON UPPER(clm.claim_tcn_id) = UPPER(min_max_dt.claim_number)
--LEFT OUTER JOIN change_837_payload   pay ON  UPPER(clm.claim_tcn_id) = UPPER(pay.claimid)
--LEFT OUTER JOIN change_837_passthrough pas ON  UPPER(clm.claim_tcn_id) = UPPER(pas.claimid)
--LEFT OUTER JOIN change_837_plainout    pln ON  UPPER(clm.claim_tcn_id) = UPPER(pln.claim_number)
...
---------- Exclude Dental
    ...
    AND
---------- Diagnosis code explosion from claim table 
        (
        ARRAY(  
                clm.principal_diagnosis_code, 
                clm.other_diagnosis_code_1,
                clm.other_diagnosis_code_2,
                clm.other_diagnosis_code_3,
                clm.other_diagnosis_code_4,
                clm.other_diagnosis_code_5,
                clm.other_diagnosis_code_6,
                clm.other_diagnosis_code_7)[diag_explode.n] IS NOT NULL
           OR 
             COALESCE(  
                clm.principal_diagnosis_code, 
                clm.other_diagnosis_code_1,
                ...
                clm.other_diagnosis_code_5,
                clm.other_diagnosis_code_6,
                clm.other_diagnosis_code_7) IS NULL AND diag_explode.n = 0
        )
...
----------- NOT EXIST IN already created table
    ...
    (
        SELECT 1
        FROM change_837_01_pvt norm_01
        WHERE clm.claim_tcn_id = norm_01.claim_id
          AND norm_01.diagnosis_code
          =
            ARRAY
                (
                clm.principal_diagnosis_code, clm.other_diagnosis_code_1, clm.other_diagnosis_code_2, clm.other_diagnosis_code_3,
