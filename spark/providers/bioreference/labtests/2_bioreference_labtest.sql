SELECT
    ...
    CONCAT(txn.accessionid, '_', COALESCE(txn.orderid_patientid, 'UNAVAILABLE'), '_', txn.testid)  AS claim_id,
    COALESCE(pay.hvid, CONCAT('500_',COALESCE(txn.orderid_patientid, 'UNAVAILABLE')))       AS hvid,
    ...
    txn.testid	                                                                            AS test_ordered_std_id,
    ...
    CASE
        WHEN txn.performinglab_performed IS NOT NULL THEN "PERFORMING_LAB_NAME"
        WHEN txn.performinglab_analyzed  IS NOT NULL THEN "ANALYZING_LAB_NAME"
    ELSE NULL
    END                                                                                     AS lab_other_qual,
    COALESCE(txn.labaddress_performed, txn.labaddress_analyzed)	                            AS lab_address_1,
    COALESCE(txn.labcity_performed, txn.labcity_analyzed)	                                AS lab_city,
    VALIDATE_STATE_CODE(UPPER(COALESCE(txn.labstate_performed, txn.labstate_analyzed )))    AS lab_state,
    COALESCE(txn.labpostalcode_performed, txn.labpostalcode_analyzed)	                    AS lab_zip,
    txn.clientname_ordering_practice                                                        AS ordering_other_id,
    CASE
        WHEN txn.clientname_ordering_practice IS NOT NULL THEN "ORDERING_PRACTICE_NAME"
    ELSE NULL
    ...
    VALIDATE_STATE_CODE(UPPER(txn.clientaddresssubentity))	                                AS ordering_state,
