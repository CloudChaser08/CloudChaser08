SELECT DISTINCT
    ...
    /* effectivedate */
    CASE
        WHEN enr.effectivedate > DATE_ADD(CAST('{VDR_FILE_DT}' AS DATE), 365)  THEN NULL
        WHEN enr.effectivedate < CAST('{EARLIEST_SERVICE_DATE}' AS DATE) THEN NULL
    ...
    CASE
        WHEN enr.effectivedate > DATE_ADD(CAST('{VDR_FILE_DT}' AS DATE), 365)                THEN NULL
        WHEN enr.effectivedate < CAST('{EARLIEST_SERVICE_DATE}' AS DATE)               THEN NULL
        WHEN enr.terminationdate > CONCAT(1 + YEAR(CAST('{VDR_FILE_DT}' AS DATE)), '-12-31') THEN CONCAT(1 + YEAR(CAST('{VDR_FILE_DT}' AS DATE)), '-12-31')
