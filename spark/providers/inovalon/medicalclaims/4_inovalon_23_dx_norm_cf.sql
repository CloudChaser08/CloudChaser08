SELECT
	...
	'543'                                                                                   AS data_vendor,
	/* patient_gender */
    clm.gender AS patient_gender,
    clm.age AS patient_age,
	/* patient_year_of_birth */
	clm.yearofbirth AS patient_year_of_birth,
    clm.threedigitzip AS patient_zip3,
    clm.state AS patient_state,
    clm.claim_type,
	clm.date_received,
	clm.date_service,
  ...
  /* inst_discharge_status_std_id */
    ...
    -----------------------------------------------------------------------------------------------------------------------------
    ---- psp.npinumber added 2020-03-27
    CASE
        WHEN ( UPPER(COALESCE(clm.claimformtypecode, 'X')) = 'P' OR  UPPER(COALESCE(clm.professionaltypecode,'X'))  IN ('I','O','U') )
            AND LPAD(clm.pos_code, 2, 0) IN ('05', '06', '07', '08', '09', '12', '13', '14', '33','99') THEN NULL
        WHEN (UPPER(COALESCE(clm.claimformtypecode, 'X')) = 'I'  OR  UPPER(COALESCE(clm.institutionaltypecode,'X')) IN ('I','O','U') )
...
----------- Rendering
LEFT OUTER JOIN prv      prv_rend      ON clm.renderingprovideruid         = prv_rend.provideruid
LEFT OUTER JOIN psp      prv_rend_psp  ON clm.renderingprovideruid         = prv_rend_psp.provideruid
----------- Billing
LEFT OUTER JOIN prv      prv_bill      ON clm.billingprovideruid  = prv_bill.provideruid
LEFT OUTER JOIN psp      prv_bill_psp  ON clm.billingprovideruid  = prv_bill_psp.provideruid

----------- Provider
LEFT OUTER JOIN prv      ON clm.provideruid   = prv.provideruid

----------- Provider PSP Added 2020-03-27
LEFT OUTER JOIN psp      ON clm.provideruid   = psp.provideruid
----------- Proxy Amount
LEFT OUTER JOIN xref    ON clm.claimuid      = xref.claimuid
LEFT OUTER JOIN ipps    ON xref.dischargeuid = ipps.dischargeclaimuid
LEFT OUTER JOIN nonipps ON cLm.claimuid      = nonipps.claimuid
