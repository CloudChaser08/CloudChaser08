...
-- Previous    COALESCE(clm.patient_birth_year, pay.yearofbirth)                                       AS patientyob,
    ...
    CASE
        ...
        ELSE CLEAN_UP_NDC_CODE(COALESCE(clm.billing_prov_npi, ''))
...
FROM change_dxrelay_claims clm
LEFT OUTER JOIN change_dxrelay_service_lines sln    ON UPPER(clm.claim_tcn_id) = UPPER(sln.claim_number)
                                                    AND sln.service_line_number = 1
LEFT OUTER JOIN change_dxrelay_supplement sup       ON UPPER(clm.claim_tcn_id) = UPPER(sup.claim_number)
LEFT OUTER JOIN matching_payload pay               ON pay.hvid IS NOT NULL AND UPPER(clm.claim_tcn_id) = UPPER(pay.claimid)
LEFT OUTER JOIN change_dxrelay_plainout pln              ON UPPER(pln.claim_number) = UPPER(clm.claim_tcn_id)
LEFT OUTER JOIN
(
SELECT sln_1.claim_number, MIN(sln_1.service_from_date) AS min_service_from, MAX( sln_1.service_to_date) AS max_service_to
FROM change_dxrelay_service_lines sln_1
GROUP BY sln_1.claim_number
) sln_date ON clm.claim_tcn_id = sln_date.claim_number
-- WHERE clm.claim_tcn_id IS NOT NULL
ORDER BY 1