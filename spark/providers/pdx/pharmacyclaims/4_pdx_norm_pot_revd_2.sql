SELECT
	...
	'HIST' AS source_file_type,
	part_provider,
	part_best_date
 FROM pdx_norm_hist hst
WHERE hst.logical_delete_reason IS NULL
  AND UPPER(COALESCE(hst.response_code_vendor, '')) = 'D'
  AND UPPER(COALESCE(hst.payer_type, '')) <> 'CASH'
  ...
  AND NOT EXISTS
    (
        ...
        WHERE COALESCE(rv1.logical_delete_reason, '') = 'Reversed Claim'
          AND COALESCE(hst.rx_number, '') = COALESCE(rv1.rx_number, '')
          AND COALESCE(hst.pharmacy_other_id, '') = COALESCE(rv1.pharmacy_other_id, '')
          /* Don't match if either date is NULL. */
          AND COALESCE(hst.logical_delete_reason_date, CAST('1900-01-01' AS DATE)) =
              COALESCE(rv1.logical_delete_reason_date, CAST('1901-01-01' AS DATE))
          AND COALESCE(hst.cob_count, '') = COALESCE(rv1.cob_count, '')
          AND CAST(COALESCE(hst.paid_ingredient_cost, 0) AS FLOAT) = CAST(COALESCE(rv1.paid_ingredient_cost, 0) AS FLOAT)
          AND (CAST(COALESCE(hst.paid_patient_pay, 0) AS FLOAT) + CAST(COALESCE(hst.paid_gross_due, 0) AS FLOAT)) =
          ...
          AND
            (
             ...
             OR COALESCE(hst.ndc_code, '00000000000') = '00000000000'
             OR COALESCE(hst.ndc_code, '') = COALESCE(rv1.ndc_code, '')
            )
    )
  /* Check to see if there's already a Reversed Claim that matches this row */
  ...
  AND NOT EXISTS
    (
        ...
        WHERE COALESCE(rv2.logical_delete_reason, '') = 'Reversed Claim'
          AND COALESCE(hst.rx_number, '') = COALESCE(rv2.rx_number, '')
          AND COALESCE(hst.pharmacy_other_id, '') = COALESCE(rv2.pharmacy_other_id, '')
          /* Don't match if either date is NULL. */
          AND COALESCE(hst.logical_delete_reason_date, CAST('1900-01-01' AS DATE)) =
              COALESCE(rv2.logical_delete_reason_date, CAST('1901-01-01' AS DATE))
          AND COALESCE(hst.cob_count, '') = COALESCE(rv2.cob_count, '')
          AND CAST(COALESCE(hst.paid_ingredient_cost, 0) AS FLOAT) = CAST(COALESCE(rv2.paid_ingredient_cost, 0) AS FLOAT)
          AND (CAST(COALESCE(hst.paid_patient_pay, 0) AS FLOAT) + CAST(COALESCE(hst.paid_gross_due, 0) AS FLOAT)) =
          ...
          AND
            (
             ...
             OR COALESCE(hst.ndc_code, '00000000000') = '00000000000'
             OR COALESCE(hst.ndc_code, '') = COALESCE(rv2.ndc_code, '')
            )
    )
