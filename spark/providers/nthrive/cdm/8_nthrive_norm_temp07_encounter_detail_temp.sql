SELECT 
	...
	/* proc_dt */
	CASE
	    WHEN CAST(COALESCE(ptn_cpt.procedure_day, 'X') AS INTEGER) IS NULL
	        THEN NULL
        ELSE CAP_DATE
        	    (
        	        DATE_ADD(CAST(EXTRACT_DATE(epi.admit_dt, '%Y%m%d') AS DATE), CAST(ptn_cpt.procedure_day AS INTEGER)),
                    esdt.gen_ref_1_dt,
                    CAST(${VDR_FILE_DT} AS DATE)
        	    )
	END                                                                                     AS proc_dt,
...
/* ...AND haven't already been loaded from patient_procedure. */
  AND NOT EXISTS
    (
        SELECT 1
         FROM nthrive_norm_temp04_encounter_detail_temp ptn_prc
