...
(
    SELECT 1
    FROM inmar_04_norm_reverse_status reverse
...
(
    SELECT 1
    FROM inmar_04_norm_reversed_status reversed
    WHERE txn.claim_id = reversed.claim_id
)
--------------- No need to Insert Reverse Rows as Reverse rows are already pressnt in the combined table (JKS 9/18)
UNION ALL
(
SELECT
    txn.record_id,
    ...
    txn.part_provider,
    txn.part_best_date
FROM inmar_04_norm_reverse_status txn
)
--------------- insert reversed rows only
UNION ALL
(
    SELECT
        txn.record_id,
        ...
        txn.part_provider,
        txn.part_best_date
    FROM inmar_04_norm_reversed_status txn
)
