SELECT
    ...
    SUBSTR(TRIM(p.threeDigitZip))   AS patient_zip3,
    ...
    CASE
        WHEN SUBSTR(TRIM(t.cpn_ind), 1, 1) IN ('0', 'N') THEN 'N'
        WHEN SUBSTR(TRIM(t.cpn_ind), 1, 1) IN ('1', 'Y') THEN 'Y'
        ELSE NULL
    END                             AS event_category_flag
    CASE
        WHEN SUBSTR(TRIM(t.cpn_ind), 1, 1) IN ('0', '1', 'Y', 'N') THEN 'CNP_IND'
        ELSE NULL
    END                             AS event_category_flag_qual
FROM alliance_transactions t
INNER JOIN matching payload p ON t.agility_id = p.personid
