SELECT 
  ...
  CASE 
    ...
    -------------------------------------------------------------------------------------------------------------       
    CLEAN_UP_PROCEDURE_CODE(pvtproc.proc_cd)                               AS procedure_code,
    CASE
        WHEN CLEAN_UP_PROCEDURE_CODE(pvtproc.proc_cd) IS NOT NULL THEN pvtproc.proc_cd_qual
    ELSE NULL
    END                                                                    AS procedure_code_qual,
    
    CASE
        WHEN UPPER(COALESCE(txn.claim_type_cd, 'X')) <> 'I'                                         THEN NULL
        WHEN UPPER(COALESCE(txn.claim_type_cd, 'X')) =  'I' AND pvtproc.principal_indicator IS NULL THEN 'N'
        WHEN UPPER(COALESCE(txn.claim_type_cd, 'X')) =  'I' AND pvtproc.principal_indicator IS NOT NULL THEN principal_indicator
        ELSE 'N'
    END                                                                    AS principal_proc_ind, 
    CAST(txn.units AS FLOAT)                                               AS procedure_units_billed,
    pvtproc.proc_modfr_1	                                               AS procedure_modifier_1,
    pvtproc.proc_modfr_2	                                               AS procedure_modifier_2,
    pvtproc.proc_modfr_3	                                               AS procedure_modifier_3,
    pvtproc.proc_modfr_4	                                               AS procedure_modifier_4,
    ...
    CAST(REPLACE(txn.line_charg, '$','') AS FLOAT)	                       AS line_charge,
    CAST(REPLACE(REPLACE(txn.tot_claim_charg_amt,',',''),'$','') AS FLOAT) AS total_charge,
...
FROM txn
LEFT OUTER JOIN claimremedi_payload_dedup pay ON txn.src_claim_id = pay.claimid
LEFT OUTER JOIN claimremedi_diag_pvt pvtdiag ON txn.src_claim_id = pvtdiag.src_claim_id
LEFT OUTER JOIN claimremedi_proc_pvt pvtproc ON txn.src_claim_id = pvtproc.src_claim_id
