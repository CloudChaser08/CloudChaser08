 ...
 ----------- Reset the part_mth per svc_ln_start_dt - service start date
   CASE
	    WHEN svc_ln_start_dt is NULL
         OR (svc_ln_start_dt < CAST('{AVAILABLE_START_DATE}' AS DATE)
 	         OR   svc_ln_start_dt > CAST('{VDR_FILE_DT}'              AS DATE))
       THEN '0_PREDATES_HVM_HISTORY'
       ELSE
            SUBSTR(svc_ln_start_dt, 1, 7)
	END                                                       AS part_mth

FROM claimremedi_835_detail_pre_final
--limit 1
