SELECT 
    ...
    --  hv_clin_obsn_id
    --------------------------------------------------------------------------------------------------
    CONCAT('241_', ftg.encounter_id)														AS hv_clin_obsn_id,
    CURRENT_DATE()                                                                          AS crt_dt,
	'11'                                                                                    AS mdl_vrsn_num,
    SPLIT(ftg.input_file_name, '/')[SIZE(SPLIT(ftg.input_file_name, '/')) - 1]              AS data_set_nm,
	1801                                                                                    AS hvm_vdr_id,
	241                                                                                     AS hvm_vdr_feed_id,
	CAST(ftg.practice_id AS STRING)                                                         AS vdr_org_id,
	ftg.encounter_id												        				AS vdr_clin_obsn_id,
	CASE 
	    WHEN ftg.encounter_id IS NOT NULL THEN 'ENCOUNTER_ID' 
	    ELSE NULL 
    ...
    CONCAT('241_', ftg.encounter_id)                                                        AS hv_enc_id,
	CAP_DATE
	    (
	...
	CAP_DATE
	    (
            CAST(EXTRACT_DATE(SUBSTR(ftg.record_create_date_time, 1, 10), '%Y-%m-%d') AS DATE),
            CAST('{EARLIEST_SERVICE_DATE}' AS DATE),
            CAST('{VDR_FILE_DT}' AS DATE)
	    )																					AS data_captr_dt,
    'fatigue'                                                                               AS prmy_src_tbl_nm,
...
FROM    fatigue_score ftg
LEFT OUTER JOIN encounters enc 
                ON enc.encounter_id = ftg.encounter_id
