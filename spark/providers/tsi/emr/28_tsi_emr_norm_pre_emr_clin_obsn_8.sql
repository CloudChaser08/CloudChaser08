SELECT 
    ...
    --  hv_clin_obsn_id
    --------------------------------------------------------------------------------------------------
    CONCAT('241_', soc.encounter_id)														AS hv_clin_obsn_id,
    CURRENT_DATE()                                                                          AS crt_dt,
	'11'                                                                                    AS mdl_vrsn_num,
    SPLIT(soc.input_file_name, '/')[SIZE(SPLIT(soc.input_file_name, '/')) - 1]              AS data_set_nm,
	1801                                                                                    AS hvm_vdr_id,
	241                                                                                     AS hvm_vdr_feed_id,
	CAST(soc.practice_id AS STRING)                                                         AS vdr_org_id,
	soc.encounter_id												        				AS vdr_clin_obsn_id,
	CASE 
	    WHEN soc.encounter_id IS NOT NULL THEN 'ENCOUNTER_ID' 
	    ELSE NULL 
    ...
    CONCAT('241_', soc.encounter_id)                                                        AS hv_enc_id,
	CAP_DATE
	    (
    ...
    CASE
        WHEN clin_obsn_typ_cd_explode.n = 0     THEN 'caffeine_per_day'
        WHEN clin_obsn_typ_cd_explode.n = 1     THEN 'alcohol_amount_per_day'
        ELSE NULL
	...
	CAP_DATE
	    (
            CAST(EXTRACT_DATE(SUBSTR(soc.record_create_date_time, 1, 10), '%Y-%m-%d') AS DATE),
            CAST('{EARLIEST_SERVICE_DATE}' AS DATE),
            CAST('{VDR_FILE_DT}' AS DATE)
	    )																					AS data_captr_dt,
    'social_history'                                                                        AS prmy_src_tbl_nm,
