SELECT
   ...
   --------------------------------------------------------------------------------------------------
    ...
    --------------------------------------------------------------------------------------------------
    CASE
        WHEN CAST(EXTRACT_DATE(cen.censuseffectivedateid, '%Y%m%d') AS DATE)  < '{EARLIEST_SERVICE_DATE}'
          OR CAST(EXTRACT_DATE(cen.censuseffectivedateid, '%Y%m%d') AS DATE)  > '{VDR_FILE_DT}' THEN NULL
    ...
    --------------------------------------------------------------------------------------------------
    CASE
        WHEN CAST(EXTRACT_DATE(cen.censuseffectivedateid, '%Y%m%d') AS DATE)  < '{AVAILABLE_START_DATE}'
          OR CAST(EXTRACT_DATE(cen.censuseffectivedateid, '%Y%m%d') AS DATE)  > '{VDR_FILE_DT}'                    THEN '0_PREDATES_HVM_HISTORY'
    ELSE  CONCAT
	            (
