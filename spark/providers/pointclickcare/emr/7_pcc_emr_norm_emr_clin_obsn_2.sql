SELECT 
    ...
    obs.observation_value_imperial                                                          AS clin_obsn_msrmt,
 ...
 LEFT OUTER JOIN dobs
              ON obs.observation_id = dobs.observation_id
             AND COALESCE(obs.observation_id, '0') <> '0'
/* Only retrieve valid observation translations. */
INNER JOIN ref_gen_ref ref
        ON ref.gen_ref_domn_nm = 'pointclickcare_emr.observation'
...
/* Select only valid U.S. states and territories. */
  ...
  AND COALESCE(obs.observation_id, '0') NOT IN ('3', '4')
  /* Select only where there's a measurement. */
  AND obs.observation_value_imperial IS NOT NULL
  /* Provider selection. */
