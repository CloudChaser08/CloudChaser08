...
def extract(spark, runner, hvids, timestamp, start_dt, end_dt, pharmacy_extract):
    ...
    date_in_range = spark.createDataFrame(
            [{'cal_date' : ((start_dt + timedelta(days=i)).isoformat())} for i in xrange((end_dt - start_dt).days + 1)],
    ...
    ext = t1.join(t2, t1['data_feed'] == t2['hvm_vdr_feed_id'], 'inner') \
        ...
        .select(t1['hvid'], 'calendar_date', 'hvm_vdr_feed_id', F.lit('Y').alias('enrolled_flag'))

    # Hashing
    ext = ext.withColumn('hvid', F.md5(F.concat(t1['hvid'], F.lit('hvid'), F.lit('hv000468'), F.lit(str(timestamp)))))
