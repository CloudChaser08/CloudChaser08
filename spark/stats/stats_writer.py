...
SUMMARY_S3_OUTPUT_DIR = "s3://healthveritydev/marketplace_stats/json/{}/"
...
def _write_queries(queries, datafeed_id, version, identifier):
    """
    Write given queries to s3.

    Queries will first be written to output/<version>/ in case the s3 write fails.
    """
    output_dir = 'output/{}/'.format(version)
    ...
    for stat_name, stat_queries in queries.items():
        if len(stat_queries) > 0:
            ...
            boto3.client('s3').upload_file(
                ...
                '/'.join(S3_OUTPUT_DIR.format(version).split('/')[3:]) + filename
            )


def write_to_s3(stats, provider_conf, version):
    """
    ...
    _write_queries(queries, provider_conf['datafeed_id'], version, provider_conf['datatype'])
