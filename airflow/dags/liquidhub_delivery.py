...
get_tmp_dir  = lambda ds, k: TMP_PATH_TEMPLATE.format(get_group_id(ds, kwargs))
...
def do_fetch_return_file(ds, **kwargs):
    ...
    s3_url = S3_NORMALIZED_FILE_URL_TEPMLATE.format(gid)
    return_file = [f for f in 
            s3_utils.list_s3_bucket_files(S3_NORMALIZED_FILE_URL_TEPMLATE.format(gid))
