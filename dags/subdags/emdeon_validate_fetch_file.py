...
def do_is_valid_new_file(ds, **kwargs):
    ...
    recent_files = sorted(all_files, key=lambda f: f.st_mtime)[-kwargs['daily_num_files']:]
    ...
    if len(filter(lambda f: f.filename == kwargs['expected_file_name'], recent_files)) == 0:
        return kwargs['is_not_new']

    new_file = filter(lambda f: f.filename == kwargs['expected_file_name'], recent_files)[0]
...
def do_fetch_file(ds, **kwargs):
    sftp_config = json_loads(Variable.get('Emdeon SFTP Configuration'))[kwargs['datatype']]
    with pysftp.Connection(sftp_config['source_host'], username=sftp_config['user'], password=sftp_config['password']) as conn:
        conn.get("{}/{}".format(sftp_config['source_path'], kwargs['expected_file_name']), "{}/{}".format(kwargs['tmp_path'], kwargs['expected_file_name']))
...
def do_push_raw_to_s3(ds, **kwargs):
    ...
    check_call(['aws', 's3', 'cp', "{}{}".format(kwargs['tmp_path'], kwargs['expected_file_name']), "{}".format(kwargs['s3_raw_path'])], env=env)
...
def emdeon_validate_fetch_file(parent_dag_name, child_dag_name, start_date, schedule_interval, dag_config):
    ...
    expected_file_name  = dag_config['file_name_template'].format(two_days_ago.strftime('%Y%m%d'))
    ...
    clean_up_workspace = BashOperator(
        ...
        bash_command='rm -rf {};'.format(dag_config['tmp_path']),
        dag=dag
    )
    ...
    is_valid_new_file = BranchPythonOperator(
        ...
        op_kwargs={
            ...
            'daily_num_files'    : dag_config['daily_num_files'],
    ...
    create_tmp_dir = BashOperator(
        ...
        bash_command='mkdir -p {};'.format(dag_config['tmp_path']),
        dag=dag
    )
    ...
    is_valid_new_file.set_upstream(clean_up_workspace)
