...
res = subprocess.check_output(' '.join(['aws', 's3', 'ls', 's3://salusv/incoming/pharmacyclaims/genoa/', '--recursive', '|', 'grep', '-o', 'incoming.*', '|', 'sed', "'s/\...\.bz2//'", '|', 'uniq']), shell=True)
...
unload_queue = []
for f in res.split("\n"):
    if len(re.findall('genoa/201[2-4]', f)) == 0:
        continue
    in_path = 's3://salusv/' + f
...
#    if len(unload_queue) == 14:
    out_path = 's3://salusv/warehouse/' + f
    out_path = re.sub('[^/]+$', '', out_path)
    out_path = re.sub('incoming', 'text', out_path)
    args.append('--unload_setid=\'{}\''.format(setid))
    args.append('--output_path=\'{}\''.format(out_path))

    print ' '.join(['./rsNormalizeGenoaRX.py'] + args)
    subprocess.check_call(' '.join(['./rsNormalizeGenoaRX.py'] + args), shell=True)
    time.sleep(5)
