...
ltrim(pharmacy_npi),
CASE WHEN (prov_dispensing_qual in ('1','01')) OR (prov_dispensing_qual in ('5', '05') AND regexp_count(provider_id, '^[0-9]{10}$') = 1) then provider_id else NULL end as prov_dispensing_npi,
...
CASE WHEN (prov_prescribing_qual in ('1','01')) OR (prov_prescribing_qual in ('5', '05') AND regexp_count(prescriber_id, '^[0-9]{10}$') = 1) then prescriber_id else NULL end as prov_prescribing_npi,
CASE WHEN (prov_primary_care_qual in ('1','01')) OR (prov_primary_care_qual in ('5', '05') AND regexp_count(primary_care_provider_id, '^[0-9]{10}$') = 1) then primary_care_provider_id else NULL end as prov_primary_care_npi,
ltrim(cob_count),
...
CASE WHEN (prov_dispensing_qual not in ('1','01')) AND (prov_dispensing_qual not in ('5', '05') OR regexp_count(prov_dispensing_id, '^[0-9]{10}$') = 0) then provider_id else NULL end as prov_dispensing_id,
ltrim(prov_dispensing_qual),
CASE WHEN (prov_prescribing_qual not in ('1','01')) AND (prov_prescribing_qual not in ('5', '05') OR regexp_count(prescriber_id, '^[0-9]{10}$') = 0) then prescriber_id else NULL end as prov_prescribing_id,
ltrim(prov_prescribing_qual),
CASE WHEN (prov_primary_care_qual not in ('1','01')) AND (prov_primary_care_qual not in ('5', '05') OR regexp_count(primary_care_provider_id, '^[0-9]{10}$') = 0) then primary_care_provider_id else NULL end as prov_primary_care_id,
