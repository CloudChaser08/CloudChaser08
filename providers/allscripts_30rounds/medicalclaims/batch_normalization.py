...
res = subprocess.check_output(' '.join(['aws', 's3', 'ls', 's3://salusv/incoming/medicalclaims/allscripts_30rounds/', '--recursive', '|', 'grep', 'transactions', '|', 'grep', '-o', 'incoming.*', '|', 'sed', "'s/\...\.bz2//'", '|', 'uniq']), shell=True)
